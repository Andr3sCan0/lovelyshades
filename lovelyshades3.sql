...
-- 2.3 Tabla RolPermiso (Relación muchos a muchos)
...
BEGIN
    ...
    -- Insertar detalles (aquí se procesaría el string de productos)
...
BEGIN
    ...
    INSERT INTO producto (código_interno, nombre_produto, descripcion, id_marca, id_categoria, color, valor_venta, valor_costo, stock_actual, fecha_ingreso, estado)
...
-- 5.10 Consultar producto más vendido
...
SELECT 
    ...
    CASE WHEN i.id_servicio IS NOT NULL THEN 'Servicio #' + s.código_interno ELSE NULL END AS servicio
FROM Inventario i
INNER JOIN producto p ON i.id_producto = p.Id_produto
